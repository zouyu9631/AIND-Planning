...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if a_node.prenodes.issubset(self.s_levels[level]):
                ...
                # check if mutex in a_node.prenodes. ??? Is this necessary ???
                is_action_support_mutex = False
                # prenodes_in_level = [node for node in self.s_levels[level] if node in a_node.prenodes]
                # for i, n1 in enumerate(prenodes_in_level[:-1]):
                #     for n2 in prenodes_in_level[i + 1:]:
                #         if n1.is_mutex(n2):
                #             is_action_support_mutex = True

                # if not mutex in a_node.prenodes, this action could be added in this level
                if not is_action_support_mutex:
    ...
    def add_literal_level(self, level):
        ...
        # then connect parents with children
        for a_node in self.a_levels[level - 1]:
            for s_node in self.s_levels[level]:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        return not (set(node_a1.action.effect_add).isdisjoint(set(node_a2.action.effect_rem)) and
                    set(node_a1.action.effect_rem).isdisjoint(set(node_a2.action.effect_add)))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        return not (set(node_a1.action.effect_add).isdisjoint(set(node_a2.action.precond_neg)) and
                    set(node_a1.action.effect_rem).isdisjoint(set(node_a2.action.precond_pos)) and
                    set(node_a2.action.effect_add).isdisjoint(set(node_a1.action.precond_neg)) and
                    set(node_a2.action.effect_rem).isdisjoint(set(node_a1.action.precond_pos))
                    )
    ...
    def h_levelsum(self) -> int:
        ...
        for literal in self.problem.goal:
            for level, nodes in enumerate(self.s_levels):
                if literal in [node.literal for node in nodes]:
                    level_sum += level
                    break
        return level_sum
    ...
    def h_setlevel(self) -> int:
        '''
        the set-level heuristic finds the level at which all the literals in the conjunctive goal appear in the
        planning graph without any pair of them being mutually exclusive.

        :return: int
        '''
        goal_set = set(self.problem.goal)
        for level, nodes in enumerate(self.s_levels):
            if goal_set.issubset(set([node.literal for node in nodes])):
                nodes_of_goal = [node for node in nodes if node.literal in goal_set]
                has_mutex = False
                for i, n1 in enumerate(nodes_of_goal[:-1]):
                    for n2 in nodes_of_goal[i + 1:]:
                        if n1.is_mutex(n2):
                            has_mutex = True
                if not has_mutex:
                    return level
        ...
        # for level, nodeset in enumerate(self.s_levels):
        #     for node in nodeset:
